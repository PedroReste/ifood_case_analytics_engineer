...
- **`KEY_CONTRACTS`**: declara sete grains (chaves) usados pelos joins: pedido, customer, item composto, pagamento composto, produto, seller e tradução de categoria.
...
- **`SOURCE_FILES`**: contrato nominal dos nove CSVs obrigatórios, em ordem determinística.
- **`PipelinePaths`**: centraliza caminhos de `bronze/`, `silver/`, `gold/` e `reports/` na própria orquestração.
...
- **`fact_orders`**: grain de um `order_id`; traz cliente, status, datas, GMV de itens, frete, pagamento, review e SLA.
- **`fact_order_items`**: grain de `(order_id, order_item_id)`; traz produto, seller, categoria, preço e frete. `LEFT JOINs` preservam a evidência mesmo diante de uma dimensão órfã, que continua visível no DQ.
...
- **`bootstrap_clustered_mean_difference()`**: função mantida no próprio notebook para reduzir arquivos do repositório sem alterar o método estatístico usado no EDA.
...
- **`collect_metrics()`**: lê somente agregados, monta período, GMV, categorias e seller concentration; facts, IDs e comentários não entram no payload.
- **`local_summary()`**: fallback determinístico, seguro para payload sem categoria e formatado para português.
- **`summarize()`**: usa o fallback sem credencial; com `OPENAI_API_KEY`, envia apenas o payload agregado à Responses API e limita o prompt a um rascunho sem claims de causality.
...
4. Distribuições, sensitivity analysis e participação financeira dos outliers.
5. Grain e candidate keys, nulidade condicionada ao status e temporal consistency.
...
3. Seasonality, comparação por mês de calendário e cobertura temporal.
4. Categorias por GMV, frete, review e atraso, com SQL DuckDB.
5. Delivery/recompra: bootstrap agrupado por cliente e sensitivity analysis de 90 dias.
...
- **`tests/test_pipeline.py` e `tests/test_ai_summary.py`**: adicionam `src/` ao caminho de teste sem exigir instalação editável.
